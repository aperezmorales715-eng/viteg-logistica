...
if st.session_state.rol is None:
    ...
    with col2:
        ...
        st.title("💧 Agua VITEG")
        st.markdown("### Sistema de Gestión Logística")
        st.markdown("---")
...
@contextmanager
def get_db():
    """Context manager para conexiones a la base de datos con mejor manejo de errores."""
    db = None
    try:
...
def get_pedidos_cached():
    """Obtiene todos los pedidos con caché para mejorar rendimiento."""
    with get_db() as db:
        if db:
            try:
                return pd.read_sql("SELECT * FROM pedidos", db)
            except Exception as e:
...
def invalidar_cache():
    """Invalida el caché de pedidos."""
    get_pedidos_cached.clear()

# ==========================================
# FUNCIONES AUXILIARES MEJORADAS
# ==========================================
def reiniciar_ruta_completa(nombre_ruta: str) -> bool:
    """Reinicia todos los pedidos de una ruta específica."""
    with get_db() as db:
        if db:
            try:
                cursor = db.cursor()
...
def extraer_foto_y_texto(referencia: str) -> Tuple[str, Optional[str]]:
    """Extrae URLs de fotos del texto de referencia."""
    if not referencia:
        return "", None
    try:
...
def exportar_excel(df: pd.DataFrame) -> bytes:
    """Exporta un DataFrame a Excel en memoria."""
    try:
        output = io.BytesIO()
...
def optimizar_ruta(df: pd.DataFrame) -> pd.DataFrame:
    """Optimiza la ruta usando el algoritmo del vecino más cercano mejorado."""
    if df.empty:
        return df
    ...
    try:
        ...
        # Comenzar desde el punto más cercano al centro
...
def generar_resumen_ruta(df: pd.DataFrame) -> Dict:
    """Genera un resumen estadístico de una ruta."""
    try:
        total = len(df)
        pendientes = len(df[df['estatus'] == 'pendiente'])
        entregados = len(df[df['estatus'] == 'entregado'])
        no_encontrados = len(df[df['estatus'] == 'no encontrado'])
        total_garrafones = df['cantidad_20L'].sum() + df['cantidad_10L'].sum()
        ...
        return {
            ...
            'total_garrafones': int(total_garrafones)
        }
    except Exception:
        return {
            'total': 0,
...
# Crear botones de navegación con mejor diseño
...
def check_nuevos_pedidos():
    """Verifica si hay nuevos pedidos en la base de datos."""
    with get_db() as db:
        if db:
            try:
                cursor = db.cursor()
                ...
                cursor.close()
                return total or 0, max_id or 0
            except:
                return 0, 0
    return 0, 0

total_actual, id_max_actual = check_nuevos_pedidos()
            ...
            if db:
                try:
                    ...
                    if not df_notif.empty:
                        ...
                        # Mostrar clientes con botones de acción
                ...
                try:
                    ...
                    if not df_rep.empty:
                        ...
                        if not df_entregados.empty:
                            ...
                            # Exportar ventas por repartidor
