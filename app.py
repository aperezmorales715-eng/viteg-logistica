...
# IMPORTAR SQLITE
# ==========================================
from database import get_db, inicializar_bd, registrar_accion_sqlite
...
def registrar_accion(usuario, accion):
    registrar_accion_sqlite(usuario, accion)
...
def reiniciar_ruta_completa(nombre_ruta: str) -> bool:
    with get_db() as conn:
        if conn:
            try:
                conn.execute(
                    "UPDATE pedidos SET estatus = 'pendiente' WHERE ruta = ?",
                    (nombre_ruta,)
                )
                conn.commit()
                return True
...
with st.sidebar:
    ...
    with get_db() as conn:
        if conn:
            try:
                cursor = conn.execute("SELECT COUNT(*) FROM pedidos WHERE estatus = 'pendiente'")
                pendientes_total = cursor.fetchone()[0]
                cursor = conn.execute("SELECT COUNT(*) FROM pedidos WHERE estatus = 'entregado'")
...
with get_db() as conn:
    if conn:
        try:
            cursor = conn.execute("SELECT COUNT(id), MAX(id) FROM pedidos")
...
if st.session_state.rol == "admin":
    ...
    if seccion == "📍 Mapa":
        ...
        with get_db() as conn:
            if conn:
                try:
                    df_mapa_todo = pd.read_sql_query(
                        ...
                        with get_db() as conn2:
                            if conn2:
                                df_todos = pd.read_sql_query(
                                    "SELECT estatus FROM pedidos WHERE ruta = ?",
                                    conn2,
                                    params=(ruta_sel,)
                                )
                            ...
                            with st.expander(f"{prefix_icon} {num_parada}{row['nombre_cliente']} | {num_tel}"):
                                ...
                                with col_e1:
                                    if st.button(txt['entregado'], key=f"ent_a_{row['id']}"):
                                        conn.execute(
                                            "UPDATE pedidos SET estatus = 'entregado' WHERE id = ?",
                                            (row['id'],)
                                        )
                                        conn.commit()
                                        st.session_state.ruta_optimizada = False
                                ...
                                with col_e2:
                                    if row['estatus'] != 'no encontrado':
                                        if st.button(txt['no_encontrado'], key=f"noe_a_{row['id']}"):
                                            conn.execute(
                                                "UPDATE pedidos SET estatus = 'no encontrado' WHERE id = ?",
                                                (row['id'],)
                                            )
                                            conn.commit()
    ...
    if seccion == "📝 Registro":
        ...
        with get_db() as conn:
            if conn:
                try:
                    cursor = conn.execute("SELECT DISTINCT ruta FROM pedidos WHERE ruta IS NOT NULL AND ruta != ''")
                    for row in cursor.fetchall():
                        if row[0] not in opciones_rutas:
                            opciones_rutas.append(row[0])
                    cursor.close()
                except:
                    pass
                ...
                if conn:
                    try:
                        df_bus = pd.read_sql_query(
                            ...
                            FROM pedidos 
                            WHERE nombre_cliente LIKE ? OR telefono LIKE ?
                            """,
                            conn,
    ...
    if seccion == "📊 Administrador":
        ...
        with get_db() as conn:
            if conn:
                try:
                    df_base = pd.read_sql_query("SELECT * FROM pedidos", conn)
                            ...
                            if st.button(txt['eliminar'], use_container_width=True):
                                ...
                                if st.button("✅ Sí, eliminar", key="confirmar_eliminar"):
                                    with get_db() as conn_del:
                                        if conn_del:
                                            conn_del.execute("DELETE FROM pedidos WHERE id = ?", (id_seleccionado,))
                                            conn_del.commit()
                                ...
                                if st.form_submit_button("💾 Guardar Cambios", type="primary"):
                                    try:
                                        conn.execute(
                                            """
                                            UPDATE pedidos 
                                            SET nombre_cliente=?, telefono=?, ruta=?, 
                                                referencia=?, cantidad_20L=?, cantidad_10L=? 
                                            WHERE id=?
                                            """,
                            ...
                            if st.button("🚨 REINICIAR TODAS LAS RUTAS", use_container_width=True, type="primary"):
                                ...
                                if st.button("✅ Sí, reiniciar todo", key="confirmar_reinicio_todo"):
                                    with get_db() as conn_reset:
                                        if conn_reset:
                                            conn_reset.execute("UPDATE pedidos SET estatus = 'pendiente'")
                                            conn_reset.commit()
    ...
    if seccion == "📈 Reportes":
        ...
        with get_db() as conn:
            if conn:
                try:
                    df_rep = pd.read_sql_query("SELECT * FROM pedidos", conn)
        ...
        with get_db() as conn:
            if conn:
                try:
                    cursor = conn.execute("SELECT DISTINCT ruta FROM pedidos WHERE ruta IS NOT NULL AND ruta != ''")
                    for row in cursor.fetchall():
                        if row[0] not in opciones_rutas_r:
                            opciones_rutas_r.append(row[0])
                    cursor.close()
                except:
                    pass
...
st.caption("🟢 Base de datos: SQLite (local)")
